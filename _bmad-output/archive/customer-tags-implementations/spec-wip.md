...
**Execution:**
- [ ] `src/context/LanguageContext.tsx` -- add `tags: 'Tags'`, `addTag: 'Add tag'`, `pressEnterToAddTag: 'Press Enter or comma to add'` to `en` and equivalent PT translations to `pt`
- [ ] `src/components/TaskCard.tsx` -- render `task.tags` as a row of `Badge` chips (variant `secondary`, `text-xs`) between the time/revenue row and the due-date row; hide row entirely when `tags` is empty
- [ ] `src/components/AddTaskDialog.tsx` -- add a `tags` state (`string[]`), a text input that adds a tag on Enter/comma keypress and trims+deduplicates, renders existing tags as removable chips, and passes `tags` in the `ADD_TASK` dispatch; reset `tags` on form reset
- [ ] `src/components/TaskDetailPanel.tsx` -- add the same tag chip-input UI; dispatch `UPDATE_TASK` with updated `tags` array on every add/remove
